...
static char lang_ext[12][8] = { "c", "cc", "pas", "java", "rb", "sh", "py",
		"php", "pl", "cs", "m", "bas" };
...
void _addtestingpointinfo_mysql(int solution_id, const char* usedtimefile,const char* flagfile,const char* scorefile) {
	char sql[(1 << 16)], *end;
	char utinfo[(1 << 16)], *uend;
	FILE *fp = fopen(usedtimefile, "r");

	uend = utinfo;
	while (fgets(uend, 1024, fp)) {
		uend += strlen(uend);
		if (uend - utinfo > 40000)
			break;
	}

	char flaginfo[(1 << 16)], *flagend;
	FILE *flagfp = fopen(flagfile, "r");

	flagend = flaginfo;
	while (fgets(flagend, 1024, flagfp)) {
		flagend += strlen(flagend);
		if (flagend - flaginfo > 40000)
			break;
	}

	char scoreinfo[(1 << 16)], *scoreend;
	FILE *scorefp = fopen(scorefile, "r");

	scoreend = scoreinfo;
	while (fgets(scoreend, 1024, scorefp)) {
		uend += strlen(scoreend);
		if (scoreend - scoreinfo > 40000)
			break;
	}

	uend = 0;
	end = sql;
	strcpy(end, "UPDATE solution SET usedtime_list='");
	end += strlen(sql);
	end += mysql_real_escape_string(conn, end, utinfo, strlen(utinfo) - 1);
	end += sprintf(end, "',flag_list='");
	end += mysql_real_escape_string(conn, end, flaginfo, strlen(flaginfo) - 1);
	end += sprintf(end, "',score_list='");
	end += mysql_real_escape_string(conn, end, scoreinfo, strlen(scoreinfo) - 1);
	end += sprintf(end, "' WHERE solution_id=%d", solution_id);
	*end = 0;
...
//addby zzy 2014.05.21
void addtestingpointinfo(int solution_id, char * usedtimefile, char* flagfile, char* scorefile) {
	_addtestingpointinfo_mysql(solution_id, usedtimefile, flagfile, scorefile);
}
...
void prepare_files(char * filename, int namelen, char * infile, int & p_id,
		char * work_dir, char * outfile, char * userfile, char* usedtimefile,
		char* flagfile, char* scorefile, int runner_id) {
	...
	//addby zzy 2014.05.21,2014.05.22
	sprintf(usedtimefile, "%s/run%d/usedtime.out", oj_home, runner_id);
	sprintf(flagfile, "%s/run%d/flag.out", oj_home, runner_id);
	sprintf(scorefile, "%s/run%d/score.out", oj_home, runner_id);
...
void copy_objc_runtime(char * work_dir) {
	...
	execute_cmd("cp -aL /lib/libdbus-1.so.3                          %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /lib/libgcc_s.so.1                           %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /lib/libgcrypt.so.11                         %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /lib/libgpg-error.so.0                       %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /lib/libz.so.1                               %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /lib/tls/i686/cmov/libc.so.6                 %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /lib/tls/i686/cmov/libdl.so.2                %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /lib/tls/i686/cmov/libm.so.6                 %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /lib/tls/i686/cmov/libnsl.so.1               %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /lib/tls/i686/cmov/libpthread.so.0           %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /lib/tls/i686/cmov/librt.so.1                %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /usr/lib/libavahi-client.so.3                %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /usr/lib/libavahi-common.so.3                %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /usr/lib/libdns_sd.so.1                      %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /usr/lib/libffi.so.5                         %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /usr/lib/libgnustep-base.so.1.19             %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /usr/lib/libgnutls.so.26                     %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /usr/lib/libobjc.so.2                        %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /usr/lib/libtasn1.so.3                       %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /usr/lib/libxml2.so.2                        %s/lib/ ",
			work_dir);
	execute_cmd("cp -aL /usr/lib/libxslt.so.1                        %s/lib/ ",
			work_dir);
...
//              sprintf(java_p2, "-Xmx%dM", mem_lmt);
		...
		execl("/usr/bin/java", "/usr/bin/java", java_xms, java_xmx,
				...
				"-Djava.security.policy=../etc/java0.policy", "Main",
				(char *) NULL);
		break;
	case 4:
...
//addby zzy 2014.05.26
void generate_spj_file(char* oj_home, int problem_id) {
	char spj_source_file[BUFFER_SIZE];
	char spj_file[BUFFER_SIZE];
	sprintf(spj_source_file, "%s/data/%d/spj.cc", oj_home, problem_id);
	sprintf(spj_file, "%s/data/%d/spj", oj_home, problem_id);

	if((access(spj_file, F_OK)) != -1) { //if spj file exist
		if (access(spj_file, X_OK) == -1) { //if spj file exist but not executable
			execute_cmd("chmod +x %s", spj_file);
		}
	} else { //if spj file not exist, we should compile it and make it executable
		//if spj source file exist, we continue
		if((access(spj_source_file, F_OK)) != -1) {
			execute_cmd("g++ %s -o %s", spj_source_file, spj_file);
			//execute_cmd("chmod +x %s", spj_file);
		}
	}
}
...
int special_judge(char* oj_home, int problem_id, char *infile, char *outfile,
		char *userfile) {
	//addby zzy 2014.05.26
	generate_spj_file(oj_home, problem_id);
...
int get_sim(int solution_id, int lang, int pid, int &sim_s_id) {
	...
	if (!sim) {
		...
		//c cpp will
		if (lang == 0)
			execute_cmd("ln ../data/%d/ac/%d.%s ../data/%d/ac/%d.%s", pid,
...
int main(int argc, char** argv) {
	...
	char usedtimefile[BUFFER_SIZE];
	char flagfile[BUFFER_SIZE];
	char scorefile[BUFFER_SIZE];
	...
	for (; (oi_mode || ACflg == OJ_AC) && (dirp = readdir(dp)) != NULL;) {
		...
		prepare_files(dirp->d_name, namelen, infile, p_id, work_dir, outfile,
				userfile, usedtimefile, flagfile, scorefile, runner_id);
		init_syscalls_limits(lang);
	...
	//addby zzy 2014.05.21
	addtestingpointinfo(solution_id, usedtimefile, flagfile, scorefile);
